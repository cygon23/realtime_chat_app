...
interface Message {
  type: 'join' | 'message' | 'leave' | 'user_list' | 'typing' | 'stop_typing' | 'reaction_add' | 'reaction_remove' | 'read_receipt' | 'room_list' | 'room_created' | 'room_joined';
  userId?: string;
  username?: string;
  content?: string;
  timestamp?: number;
...
export default function ChatApp() {
    ...
    ws.onmessage = (event) => {
      ...
      switch (message.type) {
        ...
        case 'message':
          ...
          // Send read receipt for other users' messages
          if (message.username !== username && wsRef.current?.readyState === WebSocket.OPEN) {
        ...
        case 'read_receipt':
          if (message.messageId && message.username) {
            setMessages(prev => prev.map(msg => {
              if (msg.id === message.messageId) {
                const readBy = msg.readBy || [];
                if (!readBy.includes(message.username!)) {
                  return { ...msg, readBy: [...readBy, message.username!] };
                }
              }
              return msg;
            }));
        ...
        case 'room_created':
          if (message.room) {
            ...
            toast({
              title: "Room Created",
              description: `Room "${message.room.name}" has been created`,
            });
          }
          break;
        ...
        case 'room_joined':
          if (message.room) {
            ...
            toast({
              title: "Joined Room",
              description: `You joined "${message.room.name}"`,
            });
          }
          break;
      }
    ...
    messageObserverRef.current = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && wsRef.current?.readyState === WebSocket.OPEN) {
            const messageId = entry.target.getAttribute('data-message-id');
            const messageUsername = entry.target.getAttribute('data-message-username');
            
            if (messageId && messageUsername !== username) {
              wsRef.current.send(JSON.stringify({
                type: 'read_receipt',
                messageId
              }));
  ...
  useEffect(() => {
    if (!messageObserverRef.current) return;
            ...
            {messages.map((msg, index) => (
              <div key={msg.id} className="animate-message-in" style={{ animationDelay: `${index * 0.02}s` }}>
                {msg.type === 'system' ? (
                  ...
                  <div 
                    className={`group flex gap-2 sm:gap-3 ${msg.username === username ? 'flex-row-reverse' : 'flex-row'}`}
                    data-message-id={msg.id}
                    data-message-username={msg.username}
                  >
